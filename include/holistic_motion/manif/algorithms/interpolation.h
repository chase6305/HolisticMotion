#ifndef _HOLISTIC_MOTION_HOLISTIC_MOTION_INTERPOLATION_H_
#define _HOLISTIC_MOTION_HOLISTIC_MOTION_INTERPOLATION_H_
 ...
 */
template <typename T>
constexpr T BinomialCoefficient(const T n, const T k) {
    return (n >= k) ? (k >= 0) ? (k * 2 > n)
                                         ? BinomialCoefficient(n, n - k)
                                         : k ? BinomialCoefficient(n, k - 1) *
                                                           (n - k + 1) / k
                                             : 1
                               // assert n ≥ k ≥ 0
...
constexpr T PolynomialBernstein(const T n, const T i, const T t) {
    return BinomialCoefficient(n, i) * Ipow(T(1) - t, n - i) * Ipow(t, i);
}
 ...
 */
template <typename T>
T SmoothingPhi(const T t, const std::size_t degree) {
    //  if (degree < 5)
    //  {
...
/**
 * @brief Linear interpolation in SE3 space.
...
SE3<_Scalar> InterpolateLinearInSE3(const SE3<_Scalar>& ma,
                                    const SE3<_Scalar>& mb,
                                    const _Scalar t) {
    HOLISTIC_MOTION_CHECK(t >= _Scalar(0) && t <= _Scalar(1), "s must be be in [0, 1].");

    Eigen::Matrix<_Scalar, 3, 1> ta, tb, tc;
    ta = ma.GetTranslation();
    tb = mb.GetTranslation();
    tc = ta + t * (tb - ta);

    SO3<_Scalar> rot = InterpolateLinear(ma.AsSO3(), mb.AsSO3(), t);
    SE3<_Scalar> mc(tc, rot);
...
static typename LieGroupBase<_Derived>::LieGroup InterpolateLinear(
...
InterpolateSlerp(const LieGroupBase<_Derived>& ma,
                  const LieGroupBase<_Derived>& mb,
                  const _Scalar t/*,
                    ...
                    LieGroupBase<_Derived>::_*/)
{
    ...
    //    mc = ma.rplus( mb.Rminus(ma, J_rmin_mb, J_rmin_ma) * t, p1J_mc_ma,
    //    J_mc_rmin);
...
template <typename _Derived, typename _Scalar>
static typename LieGroupBase<_Derived>::LieGroup
InterpolateCubic(const LieGroupBase<_Derived>& ma,
                  const LieGroupBase<_Derived>& mb,
                  const _Scalar t,
                  const typename LieGroupBase<_Derived>::Tangent& ta =
                    LieGroupBase<_Derived>::Tangent::ZeroHelper(),
                    ...
                    LieGroupBase<_Derived>::_*/)
{
    ...
    HOLISTIC_MOTION_CHECK(interp_factor >= Scalar(0) && interp_factor <= Scalar(1),
               "s must be be in [0, 1].");
...
template <typename _Derived, typename _Scalar>
static typename LieGroupBase<_Derived>::LieGroup
InterpolateSmooth(const LieGroupBase<_Derived>& ma,
                   const LieGroupBase<_Derived>& mb,
                   const _Scalar t,
                   ...
                   typename LieGroupBase<_Derived>::OptJacobianRef J_mc_mb = LieGroupBase<_Derived>::_*/)
{
    ...
    HOLISTIC_MOTION_CHECK(m >= Scalar(1), "m >= 1 !");
    Scalar interp_factor(t);
    HOLISTIC_MOTION_CHECK(interp_factor >= Scalar(0) && interp_factor <= Scalar(1),
               "s must be be in [0, 1].");

    const auto phi = SmoothingPhi(t, m);
              ...
              LieGroupBase<_Derived>::_*/)
{
    switch (method) {
        ...
        case INTERP_METHOD::CUBIC:
            return InterpolateCubic(ma, mb, t,
                             ta, tb/*,
                             J_mc_ma, J_mc_mb*/);
        case INTERP_METHOD::CNSMOOTH:
            return InterpolateSmooth(ma, mb, t, 3,
                              ta, tb/*,
                              J_mc_ma, J_mc_mb*/);
        default:
            __HOLISTIC_MOTION_THROW("Unknown interpolation method!");
            break;
    }
...
#endif /* _HOLISTIC_MOTION_HOLISTIC_MOTION_INTERPOLATION_H_ */
