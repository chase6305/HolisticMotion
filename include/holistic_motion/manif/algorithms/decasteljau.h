#ifndef _HOLISTIC_MOTION_HOLISTIC_MOTION_DECASTELJAU_H_
#define _HOLISTIC_MOTION_HOLISTIC_MOTION_DECASTELJAU_H_
...
std::vector<typename LieGroup::LieGroup> Decasteljau(
        const std::vector<LieGroup>& trajectory,
        const unsigned int degree,
        const unsigned int k_interp,
        const bool closed_curve = false) {
    HOLISTIC_MOTION_CHECK(trajectory.size() > 2,
               "Input trajectory must have more than two points!");
    HOLISTIC_MOTION_CHECK(degree <= trajectory.size(),
               "Degree must be less or equal to the number of input points!");
    HOLISTIC_MOTION_CHECK(k_interp > 0, "k_interp must be greater than zero!");

    // Number of connected, non-overlapping segments
    const unsigned int n_segments = static_cast<unsigned int>(std::floor(
            double(trajectory.size() - degree) / double((degree - 1) + 1)));
    ...
    const unsigned int segment_k_interp =
            (degree == 2) ? k_interp : k_interp * degree;
    ...
    for (unsigned int s = 0; s < segments_control_points.size(); ++s) {
        for (unsigned int t = 1; t <= segment_k_interp; ++t) {
            ...
            for (const auto m : segments_control_points[s]) Qs.emplace_back(*m);
            ...
            for (unsigned int i = 0; i < degree - 1; ++i) {
                for (unsigned int q = 0; q < Qs.size() - 1; ++q) {
                    Qs_tmp.push_back(
                            Qs[q].Rplus(Qs[q + 1].Rminus(Qs[q]) * t_01));
                }
...
#endif /* _HOLISTIC_MOTION_HOLISTIC_MOTION_DECASTELJAU_H_ */
