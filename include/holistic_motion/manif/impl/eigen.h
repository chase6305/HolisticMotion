#ifndef _HOLISTIC_MOTION_HOLISTIC_MOTION_EIGEN_H_
#define _HOLISTIC_MOTION_HOLISTIC_MOTION_EIGEN_H_
...
#define static_assert_rows_dim(x, dim)                                        \
    static_assert(                                                            \
            static_cast<int>(                                                 \
                    std::decay<decltype(x)>::type::RowsAtCompileTime) == dim, \
            "x.rows != " #dim " .");

#define static_assert_cols_dim(x, dim)                                        \
    static_assert(                                                            \
            static_cast<int>(                                                 \
                    std::decay<decltype(x)>::type::ColsAtCompileTime) == dim, \
            "x.cols != " #dim " .");
...
#define static_assert_dim_eq(l, r)                                             \
    static_assert(                                                             \
            static_cast<int>(                                                  \
                    std::decay<decltype(l)>::type::ColsAtCompileTime) ==       \
                    static_cast<int>(                                          \
                            std::decay<decltype(r)>::type::ColsAtCompileTime), \
            "lhs.cols != rhs.cols !");                                         \
    static_assert(                                                             \
            static_cast<int>(                                                  \
                    std::decay<decltype(l)>::type::RowsAtCompileTime) ==       \
                    static_cast<int>(                                          \
                            std::decay<decltype(r)>::type::RowsAtCompileTime), \
            "lhs.rows != rhs.rows !");

#define static_assert_is_vector(x) static_assert_cols_dim(x, 1);

#define static_assert_vector_dim(x, dim) \
    static_assert_is_vector(x);          \
    static_assert_rows_dim(x, dim);

#define static_assert_is_colmajor_vector(x) static_assert_rows_dim(x, 1);
...
#define assert_rows_dim(x, dim)                                                \
    static_assert(static_cast<int>(                                            \
                          std::decay<decltype(x)>::type::RowsAtCompileTime) == \
                                  dim ||                                       \
                          std::decay<decltype(x)>::type::RowsAtCompileTime ==  \
                                  Eigen::Dynamic,                              \
                  "x.rows != " #dim " .");                                     \
...
#define assert_cols_dim(x, dim)                                                \
    static_assert(static_cast<int>(                                            \
                          std::decay<decltype(x)>::type::ColsAtCompileTime) == \
                                  dim ||                                       \
                          std::decay<decltype(x)>::type::ColsAtCompileTime ==  \
                                  Eigen::Dynamic,                              \
                  "x.cols != " #dim " .");                                     \
...
struct traitscast<
        ...
        NewScalar> {
    using cast = Eigen::
            Matrix<NewScalar, _Rows, _Cols, _Options, _MaxRows, _MaxCols>;
};
...
typename std::enable_if<std::is_arithmetic<_Scalar>::value ||
                                internal::is_ad<_Scalar>::value,
                        Eigen::Matrix<_Scalar, 2, 2>>::type
skew(const _Scalar v) {
    return (Eigen::Matrix<_Scalar, 2, 2>() << _Scalar(0.), -v, v, _Scalar(0.))
            .finished();
}
...
typename std::enable_if<
        (internal::is_base_of_v<Eigen::MatrixBase<_Derived>, _Derived>() &&
...
skew(const Eigen::MatrixBase<_Derived>& v) {
    ...
    return (Eigen::Matrix<T, 3, 3>() << T(0.), -v(2), +v(1), +v(2), T(0.),
            -v(0), -v(1), +v(0), T(0.))
            .finished();
}
...
typename std::enable_if<
        ...
        Eigen::Matrix<typename _Derived::Scalar,
                      Eigen::Dynamic,
                      Eigen::Dynamic>>::type
skew(const Eigen::MatrixBase<_Derived>& v) {
    using T = typename _Derived::Scalar;
    ...
    if (v.rows() == 1) {
        ...
        __HOLISTIC_MOTION_THROW("Unexpected vector size in function skew.");
    }
}
...
Eigen::Matrix<Scalar, 3, 1> randPointInBall(Scalar radius) {
    ...
    // random(0, 2pi)
    Scalar phi = static_cast<Scalar>(rand()) /
                 (static_cast<Scalar>(RAND_MAX / (Scalar(2) * HOLISTIC_MOTION_PI)));
    // random(-1, 1)
    Scalar costheta =
            Scalar(-1) + static_cast<Scalar>(rand()) /
                                 (static_cast<Scalar>(RAND_MAX / Scalar(2)));
    // random(0, 1)
    ...
    return Eigen::Matrix<Scalar, 3, 1>(rsintheta * cos(phi),
                                       rsintheta * sin(phi), r * costheta);
}
...
Eigen::Quaternion<Scalar> randQuat() {
    ...
    return Eigen::Quaternion<Scalar>(a * sin(u2), a * cos(u2), b * sin(u3),
                                     b * cos(u3));
...
#endif /* _HOLISTIC_MOTION_HOLISTIC_MOTION_EIGEN_H_ */
