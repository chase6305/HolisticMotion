#ifndef _HOLISTIC_MOTION_HOLISTIC_MOTION_SE2_H_
#define _HOLISTIC_MOTION_HOLISTIC_MOTION_SE2_H_

#include "holistic_motion/manif/impl/se2/SE2_base.h"
...
public:
    ...
    SE2() { data_ << Scalar(0), Scalar(0), Scalar(1), Scalar(0); }

    ~SE2() = default;

    HOLISTIC_MOTION_COPY_CONSTRUCTOR(SE2)
    HOLISTIC_MOTION_MOVE_CONSTRUCTOR(SE2)

    // Copy constructor
    template <typename _DerivedOther>
    SE2(const LieGroupBase<_DerivedOther>& o);

    HOLISTIC_MOTION_GROUP_ASSIGN_OP(SE2)
     ...
     * This is used to speed up certain methods of Transform, especially
     * inverse().
     */
...
HOLISTIC_MOTION_EXTRA_GROUP_TYPEDEF(SE2)

template <typename _Scalar>
template <typename _DerivedOther>
SE2<_Scalar>::SE2(const LieGroupBase<_DerivedOther>& o) : SE2(o.Coeffs()) {
    //
}

template <typename _Scalar>
SE2<_Scalar>::SE2(const Translation& t, const std::complex<Scalar>& c)
...
template <typename _Scalar>
SE2<_Scalar>::SE2(const Scalar x,
                  const Scalar y,
                  const Scalar real,
                  const Scalar imag)
...
SE2<_Scalar>::SE2(const Eigen::Transform<_Scalar, 2, Eigen::Isometry>& h)
    : SE2(h.translation().x(),
          h.translation().y(),
          Eigen::Rotation2D<Scalar>(h.rotation()).angle()) {
    //
}

template <typename _Scalar>
typename SE2<_Scalar>::DataType& SE2<_Scalar>::Coeffs() {
    return data_;
}

template <typename _Scalar>
const typename SE2<_Scalar>::DataType& SE2<_Scalar>::Coeffs() const {
    return data_;
}
}  // namespace robotics
}  // namespace holistic_motion

#endif /* _HOLISTIC_MOTION_HOLISTIC_MOTION_SE2_H_ */
