#ifndef _HOLISTIC_MOTION_HOLISTIC_MOTION_BUNDLETANGENT_H_
#define _HOLISTIC_MOTION_HOLISTIC_MOTION_BUNDLETANGENT_H_

#include "holistic_motion/manif/impl/bundle/BundleTangent_base.h"
...
struct traits<BundleTangent<_Scalar, _T...>> {
    ...
    static constexpr std::array<int, sizeof...(_T)> DoFIdx =
            compute_indices<_T<_Scalar>::Tangent::DoF...>();
    static constexpr std::array<int, sizeof...(_T)> RepSizeIdx =
            compute_indices<_T<_Scalar>::Tangent::RepSize...>();
    static constexpr std::array<int, sizeof...(_T)> AlgIdx = compute_indices<
            _T<_Scalar>::Tangent::LieAlg::RowsAtCompileTime...>();
    ...
    static constexpr int RepSize =
            accumulate(int(_T<_Scalar>::Tangent::RepSize)...);
    ...
    using LieAlg = SquareMatrix<
            Scalar,
            accumulate(
                    int(_T<_Scalar>::Tangent::LieAlg::RowsAtCompileTime)...)>;
};

template <typename _Scalar, template <typename> class... _T>
const constexpr std::array<int, sizeof...(_T)>
        traits<BundleTangent<_Scalar, _T...>>::DoFIdx;
template <typename _Scalar, template <typename> class... _T>
const constexpr std::array<int, sizeof...(_T)>
        traits<BundleTangent<_Scalar, _T...>>::RepSizeIdx;
template <typename _Scalar, template <typename> class... _T>
const constexpr std::array<int, sizeof...(_T)>
        traits<BundleTangent<_Scalar, _T...>>::AlgIdx;
template <typename _Scalar, template <typename> class... _T>
...
private:
    static_assert(sizeof...(_T) > 0,
                  "Must have at least one element in BundleTangent !");
...
public:
    ...
    HOLISTIC_MOTION_COPY_CONSTRUCTOR(BundleTangent)
    HOLISTIC_MOTION_MOVE_CONSTRUCTOR(BundleTangent)

    // Copy constructors given base
    template <typename _DerivedOther>
    BundleTangent(const TangentBase<_DerivedOther>& o);

    HOLISTIC_MOTION_TANGENT_ASSIGN_OP(BundleTangent)
...
protected:
    ...
    template <int... _Idx>
    BundleTangent(internal::intseq<_Idx...>,
...
#endif  // _HOLISTIC_MOTION_HOLISTIC_MOTION_BUNDLETANGENT_H_
