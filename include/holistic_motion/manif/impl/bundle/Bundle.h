#ifndef _HOLISTIC_MOTION_HOLISTIC_MOTION_BUNDLE_H_
#define _HOLISTIC_MOTION_HOLISTIC_MOTION_BUNDLE_H_

#include "holistic_motion/manif/impl/bundle/Bundle_base.h"
...
struct traits<Bundle<_Scalar, _T...>> {
    ...
    static constexpr std::array<int, sizeof...(_T)> DimIdx =
            compute_indices<_T<_Scalar>::Dim...>();
    static constexpr std::array<int, sizeof...(_T)> DoFIdx =
            compute_indices<_T<_Scalar>::DoF...>();
    static constexpr std::array<int, sizeof...(_T)> RepSizeIdx =
            compute_indices<_T<_Scalar>::RepSize...>();
    static constexpr std::array<int, sizeof...(_T)> TraIdx = compute_indices<
            _T<_Scalar>::Transformation::RowsAtCompileTime...>();
...
template <typename _Scalar, template <typename> class... _T>
const constexpr std::array<int, sizeof...(_T)>
        traits<Bundle<_Scalar, _T...>>::DimIdx;
template <typename _Scalar, template <typename> class... _T>
const constexpr std::array<int, sizeof...(_T)>
        traits<Bundle<_Scalar, _T...>>::DoFIdx;
template <typename _Scalar, template <typename> class... _T>
const constexpr std::array<int, sizeof...(_T)>
        traits<Bundle<_Scalar, _T...>>::RepSizeIdx;
template <typename _Scalar, template <typename> class... _T>
const constexpr std::array<int, sizeof...(_T)>
        traits<Bundle<_Scalar, _T...>>::TraIdx;
template <typename _Scalar, template <typename> class... _T>
...
private:
    static_assert(sizeof...(_T) > 0,
                  "Must have at least one element in Bundle !");
...
public:
    ...
    HOLISTIC_MOTION_COPY_CONSTRUCTOR(Bundle)
    HOLISTIC_MOTION_MOVE_CONSTRUCTOR(Bundle)

    // Copy constructor
    template <typename _DerivedOther>
    Bundle(const LieGroupBase<_DerivedOther>& o);

    HOLISTIC_MOTION_GROUP_ASSIGN_OP(Bundle)
...
template <typename _Scalar, template <typename> class... _T>
template <int... _Idx>
Bundle<_Scalar, _T...>::Bundle(internal::intseq<_Idx...>,
                               const _T<_Scalar>&... elements) {
    // c++11 "fold expression"
    auto l = {
            ((data_.template segment<Element<_Idx>::RepSize>(std::get<_Idx>(
                      internal::traits<Type>::RepSizeIdx)) = elements.Coeffs()),
             0)...};
    static_cast<void>(l);  // compiler warning
}

template <typename _Scalar, template <typename> class... _T>
typename Bundle<_Scalar, _T...>::DataType& Bundle<_Scalar, _T...>::Coeffs() {
    return data_;
}
...
#endif  // _HOLISTIC_MOTION_HOLISTIC_MOTION_BUNDLE_H_
