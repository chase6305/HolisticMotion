#ifndef _HOLISTIC_MOTION_HOLISTIC_MOTION_SE_2_3_H_
#define _HOLISTIC_MOTION_HOLISTIC_MOTION_SE_2_3_H_

#include "holistic_motion/manif/impl/se_2_3/SE_2_3_base.h"
...
public:
    ...
    HOLISTIC_MOTION_COPY_CONSTRUCTOR(SE_2_3)
    HOLISTIC_MOTION_MOVE_CONSTRUCTOR(SE_2_3)

    template <typename _DerivedOther>
    SE_2_3(const LieGroupBase<_DerivedOther>& o);

    HOLISTIC_MOTION_GROUP_ASSIGN_OP(SE_2_3)

    /**
     * @brief Constructor given a translation, a unit quaternion and a linear
     * velocity.
    ...
    /**
     * @brief Constructor given a translation, an angle axis and a linear
     * velocity.
     * @param[in] t A translation vector.
    ...
    /**
     * @brief Constructor given a translation, SO3 element and a linear
     * velocity.
     * @param[in] t A translation vector.
    ...
    /**
     * @brief Constructor from a 3D Eigen::Isometry<Scalar> relevant to SE(3)
     ...
     */
    SE_2_3(const Eigen::Transform<_Scalar, 3, Eigen::Isometry>& h,
           const LinearVelocity& v);
...
HOLISTIC_MOTION_EXTRA_GROUP_TYPEDEF(SE_2_3)

template <typename _Scalar>
template <typename _DerivedOther>
SE_2_3<_Scalar>::SE_2_3(const LieGroupBase<_DerivedOther>& o)
    : SE_2_3(o.Coeffs()) {
    //
}
...
template <typename _Scalar>
SE_2_3<_Scalar>::SE_2_3(const Scalar x,
                        const Scalar y,
                        const Scalar z,
                        const Scalar roll,
                        const Scalar pitch,
...
SE_2_3<_Scalar>::SE_2_3(const Translation& t,
                        const SO3<Scalar>& so3,
                        const LinearVelocity& v)
    : SE_2_3(t, so3.GetQuat(), v) {
    //
}

template <typename _Scalar>
SE_2_3<_Scalar>::SE_2_3(const Eigen::Transform<_Scalar, 3, Eigen::Isometry>& h,
                        const LinearVelocity& v)
    : SE_2_3(h.GetTranslation(),
             Eigen::Quaternion<_Scalar>(h.GetRotation()),
             v) {
    //
}

template <typename _Scalar>
typename SE_2_3<_Scalar>::DataType& SE_2_3<_Scalar>::Coeffs() {
    return data_;
}

template <typename _Scalar>
const typename SE_2_3<_Scalar>::DataType& SE_2_3<_Scalar>::Coeffs() const {
    return data_;
}
}  // namespace robotics
}  // namespace holistic_motion

#endif /* _HOLISTIC_MOTION_HOLISTIC_MOTION_SE_2_3_H_ */
