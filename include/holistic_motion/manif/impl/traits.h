#ifndef _HOLISTIC_MOTION_HOLISTIC_MOTION_TRAITS_H_
#define _HOLISTIC_MOTION_HOLISTIC_MOTION_TRAITS_H_
...
//!
//! @note e.g. template <typename NewScalar> using LieGroupTemplate = typename
//! traitscast<LieGroup, NewScalar>::cast;
 ...
 */
template <template <typename, template <typename> class...> class _Class,
          typename _Scalar,
          typename _NewScalar,
          template <typename>
          class... Ts>
 ...
 */
template <template <typename, unsigned int> class _Class,
          typename _NewScalar,
          typename _Scalar,
          unsigned int _Dim>
...
struct is_ad : std::integral_constant<bool, false> {};

//! @brief Has function 'rjacinv' traits
template <class, typename T>
struct has_rjacinv_impl : std::false_type {};
template <typename T>
struct has_rjacinv_impl<decltype(void(std::declval<T>().rjacinv())), T>
    : std::true_type {};
template <typename T>
struct has_rjacinv : has_rjacinv_impl<void, T> {};

//! @brief Has function 'ljacinv' traits
template <class, typename T>
struct has_ljacinv_impl : std::false_type {};
template <typename T>
struct has_ljacinv_impl<decltype(void(std::declval<T>().ljacinv())), T>
    : std::true_type {};
template <typename T>
struct has_ljacinv : has_ljacinv_impl<void, T> {};
...
struct make_intseq {
    using type = typename intseq_join<typename make_intseq<_N - 1>::type,
                                      intseq<_N - 1>>::type;
};
...
template <class, typename T>
struct is_holistic_motion_group_impl : std::false_type {};
template <typename T>
struct is_holistic_motion_group_impl<decltype(test_lie_group_base(std::declval<T>())), T>
    : std::true_type {};
template <typename T>
struct is_holistic_motion_group : is_holistic_motion_group_impl<void, T> {};
...
template <class, typename T>
struct is_holistic_motion_tangent_impl : std::false_type {};
template <typename T>
struct is_holistic_motion_tangent_impl<decltype(test_tangent_base(std::declval<T>())), T>
    : std::true_type {};
template <typename T>
struct is_holistic_motion_tangent : is_holistic_motion_tangent_impl<void, T> {};

}  // namespace internal
}  // namespace robotics
}  // namespace holistic_motion

#endif /* _HOLISTIC_MOTION_HOLISTIC_MOTION_TRAITS_H_ */
