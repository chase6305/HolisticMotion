#ifndef _HOLISTIC_MOTION_HOLISTIC_MOTION_LIE_GROUP_BASE_H_
#define _HOLISTIC_MOTION_HOLISTIC_MOTION_LIE_GROUP_BASE_H_

#include "holistic_motion/manif/external/tl/tl/optional.hpp"
#include "holistic_motion/manif/constants.h"
#include "holistic_motion/manif/impl/assignment_assert.h"
...
struct LieGroupBase {
    ...
    using LieGroupTemplate =
            typename internal::traitscast<LieGroup, _Scalar>::cast;
...
protected:
    HOLISTIC_MOTION_DEFAULT_CONSTRUCTOR(LieGroupBase)
...
public:
     ...
     * @note This is a special case of the templated operator=. Its purpose is
    ...
    /**
     * @brief Set the LieGroup object this to a zero value.
     * @return A reference to this.
     * @note set zero
     */
    _Derived& SetZero();
     ...
     * @param[out] -optional- J_vout_v Jacobian of the new object wrt input
     * object.
     ...
     * @param[out] -optional- J_mout_t Jacobian of the oplus operation wrt the
     * tangent element.
     ...
     * @param[out] -optional- J_t_mb Jacobian of the ominus operation wrt the
     * other element.
    ...
    auto operator[](const unsigned int i) const -> decltype(Coeffs()[i]) {
        HOLISTIC_MOTION_CHECK((static_cast<int>(i) >= 0 && i < Coeffs().size()),
                   "Index out of range");
        return Coeffs()[i];
    }
    ...
    static LieGroup IdentityHelper();
    //! Static helper to create a random object of the Lie group.
    static LieGroup RandomHelper();
...
template <typename _Derived>
_Derived& LieGroupBase<_Derived>::operator=(const LieGroupBase& m) {
...
template <typename _Derived>
template <typename _DerivedOther>
_Derived& LieGroupBase<_Derived>::operator=(
...
template <typename _EigenDerived>
_Derived& LieGroupBase<_Derived>::operator=(
        const Eigen::MatrixBase<_EigenDerived>& data) {
    internal::AssignmentEvaluator<typename internal::traits<_Derived>::Base>()
            .run(data);
...
template <typename _Derived>
typename LieGroupBase<_Derived>::DataType& LieGroupBase<_Derived>::Coeffs() {
...
template <typename _Derived>
typename LieGroupBase<_Derived>::Scalar* LieGroupBase<_Derived>::Data() {
    return Derived().Coeffs().data();
}

template <typename _Derived>
const typename LieGroupBase<_Derived>::Scalar* LieGroupBase<_Derived>::Data()
        const {
    return Derived().Coeffs().data();
}
...
template <typename _Derived>
_Derived& LieGroupBase<_Derived>::SetIdentity() {
    const static Tangent zero = Tangent::ZeroHelper();
    Derived() = zero.Exp();
    return Derived();
}

template <typename _Derived>
_Derived& LieGroupBase<_Derived>::SetRandom() {
...
template <typename _Derived>
_Derived& LieGroupBase<_Derived>::SetZero() {
    Derived() = DataType::Zero();
    return Derived();
}

template <typename _Derived>
typename LieGroupBase<_Derived>::LieGroup LieGroupBase<_Derived>::Inverse(
...
template <typename _Derived>
template <typename _DerivedOther>
typename LieGroupBase<_Derived>::LieGroup LieGroupBase<_Derived>::Lplus(
        const TangentBase<_DerivedOther>& t,
        OptJacobianRef J_mout_m,
        OptJacobianRef J_mout_t) const {
    if (J_mout_t) {
...
template <typename _Derived>
template <typename _DerivedOther>
typename LieGroupBase<_Derived>::LieGroup LieGroupBase<_Derived>::Plus(
        const TangentBase<_DerivedOther>& t,
        OptJacobianRef J_mout_m,
        OptJacobianRef J_mout_t) const {
...
template <typename _Derived>
template <typename _DerivedOther>
typename LieGroupBase<_Derived>::Tangent LieGroupBase<_Derived>::Rminus(
        const LieGroupBase<_DerivedOther>& m,
        OptJacobianRef J_t_ma,
        OptJacobianRef J_t_mb) const {
...
template <typename _Derived>
template <typename _DerivedOther>
typename LieGroupBase<_Derived>::Tangent LieGroupBase<_Derived>::Lminus(
        const LieGroupBase<_DerivedOther>& m,
        OptJacobianRef J_t_ma,
        OptJacobianRef J_t_mb) const {
...
template <typename _Derived>
template <typename _DerivedOther>
typename LieGroupBase<_Derived>::Tangent LieGroupBase<_Derived>::Minus(
        const LieGroupBase<_DerivedOther>& m,
        OptJacobianRef J_t_ma,
        OptJacobianRef J_t_mb) const {
...
template <typename _Derived>
typename LieGroupBase<_Derived>::Tangent LieGroupBase<_Derived>::Log(
        OptJacobianRef J_t_m) const {
    return Derived().Log(J_t_m);
}

template <typename _Derived>
typename LieGroupBase<_Derived>::Tangent LieGroupBase<_Derived>::Lift(
        OptJacobianRef J_t_m) const {
    return Derived().Log(J_t_m);
}
...
template <typename _Derived>
template <typename _DerivedOther>
typename LieGroupBase<_Derived>::LieGroup LieGroupBase<_Derived>::Compose(
        const LieGroupBase<_DerivedOther>& m,
        OptJacobianRef J_mc_ma,
        OptJacobianRef J_mc_mb) const {
...
template <typename _Derived>
template <typename _DerivedOther>
typename LieGroupBase<_Derived>::LieGroup LieGroupBase<_Derived>::Between(
        const LieGroupBase<_DerivedOther>& m,
        OptJacobianRef J_mc_ma,
        OptJacobianRef J_mc_mb) const {
...
bool LieGroupBase<_Derived>::IsApprox(const LieGroupBase<_DerivedOther>& m,
    ...
    return Rminus(m).IsApprox(Tangent::ZeroHelper(), eps);
}

template <typename _Derived>
typename LieGroupBase<_Derived>::Jacobian LieGroupBase<_Derived>::Adj() const {
    return Derived().Adj();
}

template <typename _Derived>
template <typename _EigenDerived>
typename LieGroupBase<_Derived>::Vector LieGroupBase<_Derived>::Act(
...
template <typename _Derived>
template <typename _DerivedOther>
typename LieGroupBase<_Derived>::LieGroup LieGroupBase<_Derived>::operator+(
...
template <typename _Derived>
template <typename _DerivedOther>
typename LieGroupBase<_Derived>::Tangent LieGroupBase<_Derived>::operator-(
        const LieGroupBase<_DerivedOther>& m) const {
...
LieGroupBase<_Derived>::IdentityHelper() {
...
template <typename _Derived>
typename LieGroupBase<_Derived>::LieGroup
LieGroupBase<_Derived>::RandomHelper() {
...
_Stream& operator<<(_Stream& s,
                    const holistic_motion::robotics::LieGroupBase<_Derived>& m) {
...
#endif /* _HOLISTIC_MOTION_HOLISTIC_MOTION_LIE_GROUP_BASE_H_ */
