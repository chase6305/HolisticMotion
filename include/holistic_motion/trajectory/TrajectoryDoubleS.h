...
class TrajectoryDoubleS : public TrajectoryBase<LieGroup> {
private:
    using Tangent = typename LieGroup::Tangent;

public:
    virtual ~TrajectoryDoubleS() {
        holistic_motion::utility::LogDebug("Destructing TrajectoryDoubleS.");
    };

    /// \brief DoubleS profile
    ///
    ...
    TrajectoryDoubleS(const std::shared_ptr<PathBase<LieGroup>>& path,
...
protected:
    ...
    /// \param allow_concave
    /// \return bool
    static bool _ComputeDoubleSProfile(const double& q0,
                                       const double& q1,
                                       double& v0,
                                       double& v1,
                                       ...
                                       const int& seg_no,
                                       const bool& allow_concave = false);

    /// \brief compute TrajectoryDoubleS trajectory
    ...
    static TrajectorySeg _ComputeNextTrajStep(const TrajectorySeg& segment,
                                              const double& t,
