...
The dual-arm Viser demo defaults to the Marvin model under the local
`HumanoidAssets` repository. Drag either end-effector gizmo to solve IK and
run left-arm/right-arm geometry checks in real time. Each complete-arm group
