...
struct Tree {
    ...
    mutable detail::JointSpaceIndex index;
};
  ...
  public:
    PlanningContext(const Eigen::VectorXd &lower, const Eigen::VectorXd &upper,
        ...
        : lower_(lower), upper_(upper), metric_(weights, continuous),
          use_index_(weights.size() <= 8 && metric_.HasContinuous() &&
                     std::any_of(continuous.begin(), continuous.end(),
    ...
    Eigen::VectorXd SampleUniform() {
        ...
        for (Eigen::Index i = 0; i < sample.size(); ++i) {
            std::uniform_real_distribution<double> distribution(lower_[i],
                                                                upper_[i]);
    ...
    bool IsMotionValid(const Eigen::VectorXd &from, const Eigen::VectorXd &to) {
        if (TimedOut())
            return false;
        // Bounded interpolation stays inside the joint limits; continuous
        // joints are normalized. Without a validator, no interior state needs
        // sampling.
        if (!validator_)
            return true;
        ...
        for (std::size_t i = 1; i < segments; ++i) {
            if (TimedOut())
                return false;
            if (!IsStateValid(Normalize(
                    from + (static_cast<double>(i) / segments) * delta)) ||
                TimedOut()) {
                return false;
            }
        }
    ...
    std::size_t Nearest(const Tree &tree, const Eigen::VectorXd &state) const {
        if (use_index_)
            return tree.index.Nearest(
                tree.nodes.size(),
                [&](std::size_t index) -> const Eigen::VectorXd & {
    ...
    Near(const Tree &tree, const Eigen::VectorXd &state, double radius) const {
        if (use_index_)
            return tree.index.Near(
                tree.nodes.size(),
                [&](std::size_t index) -> const Eigen::VectorXd & {
                    return tree.nodes[index].state;
                },
                state, radius, metric_);
...
void Shortcut(std::vector<Eigen::VectorXd> &path, PlanningContext &context,
    ...
    for (std::size_t attempt = 0; attempt < attempts && path.size() >= 3;
        ...
        const auto sample_at = [&](double distance, std::size_t &segment) {
            const auto upper = std::upper_bound(cumulative.begin(),
        ...
        const Eigen::VectorXd second =
            sample_at(second_distance, second_segment);
        ...
        if (direct_length + 1e-9 >= second_distance - first_distance ||
            ...
            // Check both fragments, including the newly introduced first
            // endpoint.
...
PlanRRTConnect(const Eigen::VectorXd &start, const Eigen::VectorXd &goal,
    ...
    for (; statistics.iterations < options.max_iterations &&
        ...
        if (context.Extend(first, sample, first_index) ==
...
PlanRRTStar(const Eigen::VectorXd &start, const Eigen::VectorXd &goal,
    ...
    for (; statistics.iterations < options.max_iterations &&
        ...
        // Compare new goal connections with its current cost, otherwise a stale
        // larger bound can replace an already improved path with a worse one.
...
void SamplingPlanner::SetJointWeights(const Eigen::VectorXd &weights) {
    if (weights.size() != lower_limits_.size() || !weights.allFinite() ||
        ...
        throw std::invalid_argument("joint weights must be finite, positive, "
...
PlanningResult SamplingPlanner::Plan(const Eigen::VectorXd &start,
    ...
    if (!in_bounds(goal))
        return finish(PlanningStatus::INVALID_GOAL,
                      "goal is outside joint limits");
    ...
    case SamplingAlgorithm::RRT_STAR:
        result.path = PlanRRTStar(start, goal, context, options,
                                  result.statistics, deadline, false);
        break;
    case SamplingAlgorithm::INFORMED_RRT_STAR:
        result.path = PlanRRTStar(start, goal, context, options,
                                  result.statistics, deadline, true);
        break;
    }
    if (result.path.empty()) {
        const bool timed_out = std::chrono::steady_clock::now() >= deadline;
        return finish(
            timed_out ? PlanningStatus::TIMEOUT : PlanningStatus::NO_SOLUTION,
