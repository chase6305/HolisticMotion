...
long double ExtendedCenterDistance(const Eigen::Vector3d &first,
                                   const Eigen::Vector3d &second) {
    return std::hypot(static_cast<long double>(second.x()) - first.x(),
                      static_cast<long double>(second.y()) - first.y(),
                      static_cast<long double>(second.z()) - first.z());
}

double ExtendedSignedDistance(const Eigen::Vector3d &first,
                              const Eigen::Vector3d &second,
                              double first_radius, double second_radius,
                              Eigen::Vector3d *normal) {
    // Avoid squared-length overflow/underflow and preserve clearance when a
    // large center distance only becomes representable after subtracting radii.
    const long double distance = ExtendedCenterDistance(first, second);
    if (normal && distance > 0.0L) {
        for (Eigen::Index axis = 0; axis < 3; ++axis)
            (*normal)[axis] = static_cast<double>(
                (static_cast<long double>(second[axis]) - first[axis]) /
                distance);
    }
    const double signed_distance =
        static_cast<double>(distance - first_radius - second_radius);
    if (!std::isfinite(signed_distance))
        throw std::overflow_error("sphere distance is not finite");
    return signed_distance;
}

double SignedDistance(const Eigen::Vector3d &first,
                      const Eigen::Vector3d &second, double first_radius,
                      double second_radius, Eigen::Vector3d *normal = nullptr) {
    const Eigen::Vector3d delta = second - first;
    const double squared_distance = delta.squaredNorm();
    if (std::isnormal(squared_distance)) {
        const double distance = std::sqrt(squared_distance);
        if (normal)
            *normal = delta / distance;
        const double signed_distance = distance - first_radius - second_radius;
        if (std::isfinite(signed_distance))
            return signed_distance;
    }
    return ExtendedSignedDistance(first, second, first_radius, second_radius,
                                  normal);
}
...
class SphereCollisionModel::Impl {
  public:
    Impl(const std::string &urdf_path,
         std::vector<CollisionSphere> input_spheres, bool exclude_same_link)
        ...
        if (spheres.empty())
            throw std::invalid_argument(
                "collision sphere list must not be empty");
        ...
        for (const auto &sphere : spheres) {
            ...
            if (!sphere.center.allFinite() || !std::isfinite(sphere.radius) ||
                sphere.radius <= 0.0)
                throw std::invalid_argument("sphere center and radius must be "
                                            "finite and radius positive");
            minimum_radius = std::min(minimum_radius, sphere.radius);
            maximum_radius = std::max(maximum_radius, sphere.radius);
            if (!names.insert(sphere.name).second)
                throw std::invalid_argument(
                    "duplicate collision sphere name: " + sphere.name);
    ...
    void ValidateConfiguration(const Eigen::VectorXd &q) const {
        if (q.size() != model.nq)
            throw std::invalid_argument(
                "configuration size must equal model.nq (" +
    ...
    void UpdateWorldSpheres() {
        for (std::size_t i = 0; i < spheres.size(); ++i) {
            ...
            world_spheres.row(static_cast<Eigen::Index>(i)).head<3>() =
                (placement.rotation() * spheres[i].center +
                 placement.translation())
                    .transpose();
        }
        if (!world_spheres.allFinite())
            throw std::overflow_error(
                "world sphere coordinates are not finite");
    }

    void AddProjectedPointJacobian(std::size_t sphere_index,
        ...
        pinocchio::getFrameJacobian(model, *data, frame_ids[sphere_index],
                                    ...
                                    frame_jacobian);
        // Rotate the local offset directly: subtracting two translated world
        // points can erase a small lever arm and hence its angular
        // contribution.
        const Eigen::Vector3d offset =
            data->oMf[frame_ids[sphere_index]].rotation() *
            spheres[sphere_index].center;
        Eigen::Matrix<double, 6, 1> projection;
        projection.head<3>() = normal;
        projection.tail<3>() = offset.cross(normal);
        // Project before accumulating, without allocating two 3-by-nv point
        // Jacobians and their difference on every query.
        gradient.noalias() += frame_jacobian.transpose() * projection;
    }

    void ResetPairs(bool exclude_same_link) {
        pairs.clear();
        for (std::size_t first = 0; first < spheres.size(); ++first) {
            for (std::size_t second = first + 1; second < spheres.size();
                 ++second) {
    ...
    SphereDistanceResult Distance(std::size_t pair_index) const {
        ...
        result.distance =
            SignedDistance(first_center, second_center, spheres[first].radius,
                           spheres[second].radius, &result.normal);
    ...
    double DistanceValue(std::size_t pair_index) const {
        ...
        return SignedDistance(
            world_spheres.row(static_cast<Eigen::Index>(first)).head<3>(),
            world_spheres.row(static_cast<Eigen::Index>(second)).head<3>(),
            spheres[first].radius, spheres[second].radius);
    }
    ...
    bool CollidingPairs(double security_margin, bool stop_at_first) const {
        ...
        for (const auto &[first, second] : pairs) {
            ...
            const Eigen::Vector3d second_center =
                world_spheres.row(static_cast<Eigen::Index>(second)).head<3>();
            const double threshold = spheres[first].radius +
                                     spheres[second].radius + security_margin;
            const double squared_threshold = threshold * threshold;
            bool overlaps;
            if constexpr (CheckRange) {
                overlaps =
                    std::isnormal(squared_threshold)
                        ? (second_center - first_center).squaredNorm() <=
                              squared_threshold
                        : ExtendedCenterDistance(first_center, second_center) <=
                              static_cast<long double>(spheres[first].radius) +
                                  spheres[second].radius + security_margin;
            } else {
                overlaps = (second_center - first_center).squaredNorm() <=
                           squared_threshold;
            }
            if (overlaps) {
                collision = true;
...
std::vector<SpherePairInfo> SphereCollisionModel::GetCollisionPairs() const {
    ...
    for (const auto &[first, second] : impl_->pairs) {
        result.push_back({first, second, impl_->spheres[first].name,
                          impl_->spheres[second].name,
                          impl_->spheres[first].link_name,
                          impl_->spheres[second].link_name});
    }
    return result;
}
...
std::size_t SphereCollisionModel::SetCollisionGroups(
    ...
    for (const auto &[first_group, second_group] : group_pairs) {
        ...
        if (first == resolved.end() || second == resolved.end())
            throw std::invalid_argument(
                "collision pair references an unknown group");
...
bool SphereCollisionModel::InCollision(const Eigen::VectorXd &configuration,
    ...
    impl_->Update(configuration);
    // Radii are immutable. These conservative bounds cover every active pair,
    // including after group changes, without rebuilding a per-pair cache.
    const double lower = 2.0 * impl_->minimum_radius + security_margin;
    const double upper = 2.0 * impl_->maximum_radius + security_margin;
    if (std::isnormal(lower * lower) && std::isnormal(upper * upper))
        return impl_->CollidingPairs<false>(security_margin, stop_at_first);
    return impl_->CollidingPairs<true>(security_margin, stop_at_first);
}
...
SphereDistanceGradientResult SphereCollisionModel::MinimumDistanceWithGradient(
    ...
    if (best.first_center != best.second_center) {
        impl_->AddProjectedPointJacobian(second, best.normal, result.gradient);
        impl_->AddProjectedPointJacobian(first, -best.normal, result.gradient);
    }
    return result;
}
...
SphereCollisionModel::ComputeDistances(const Eigen::VectorXd &configuration,
    ...
    for (std::size_t i = 0; i < impl_->pairs.size(); ++i) {
        // Most pairs lie outside a local clearance query. Only construct
        // centers and normals (and allocate result storage) for the pairs being
        // returned.
        if (impl_->DistanceValue(i) <= maximum_distance)
            result.push_back(impl_->Distance(i));
    }
    return result;
}
...
Eigen::VectorXd SphereCollisionModel::BatchMinimumDistances(
    ...
    if (configurations.cols() != impl_->model.nq ||
        configurations.rows() == 0 || !configurations.allFinite())
        throw std::invalid_argument(
