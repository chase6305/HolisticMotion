...
double WrappedDistance(const Eigen::VectorXd& lhs,
...
bool FEPKinematics::IsCompatible() const noexcept {
    if (joint_nodes_.size() != 8 || GetDOF() != 7) return false;
...
FEPConfiguration FEPKinematics::GetConfiguration(
        const Eigen::VectorXd& joints) const {
    if (joints.size() != 7 || !joints.allFinite()) return {};
    return {Direction(joints[1]), Direction(joints[3]), Direction(joints[5]),
            joints[6]};
}

bool FEPKinematics::SolveSeed(const SE3d& target,
                              Eigen::VectorXd seed,
    ...
    if (solution.size() != 7 || !solution.allFinite()) return false;
    ...
    const Eigen::Matrix4d delta = actual.Inverse().GetTransform() *
                                  target.GetTransform();
    const double translation_error = delta.block<3, 1>(0, 3).norm();
    const double cosine = clamp((delta.block<3, 3>(0, 0).trace() - 1.0) * 0.5,
                                -1.0, 1.0);
    if (translation_error > 1e-5 || std::acos(cosine) > 1e-5) {
        ...
        if (refinement.GetIK(target, refined, refined_seed,
                             refined_distances) &&
...
bool FEPKinematics::SolveConfiguration(
        const SE3d& target,
        const FEPConfiguration& configuration,
        const Eigen::VectorXd& seed,
        Eigen::VectorXd& solution) const {
    if (!std::isfinite(configuration.redundancy)) return false;
    Eigen::VectorXd branch_seed = seed;
    if (branch_seed.size() != 7) return false;
    const std::array<int, 3> indices{1, 3, 5};
    const std::array<int, 3> directions{
            Direction(configuration.shoulder), Direction(configuration.elbow),
    ...
    if (!SolveSeed(target, branch_seed, solution)) return false;
...
bool FEPKinematics::Solve(const SE3d& target,
                          const Eigen::VectorXd& seed,
    ...
    if (method == FEPSolveMethod::SEEDED_NUMERICAL ||
        ...
        if (SolveSeed(target, seed, candidate)) append(candidate);
        ...
        for (double delta = 0.0; delta <= M_PI + 1e-12; delta += M_PI / 36.0) {
            for (double sign : (delta == 0.0 ? std::array<double, 2>{0.0, 0.0}
                                             : std::array<double, 2>{-1.0, 1.0})) {
                Eigen::VectorXd trial = seed;
                trial[6] = clamp(seed[6] + sign * delta, lower, upper);
                Eigen::VectorXd candidate;
                if (!SolveSeed(target, trial, candidate)) continue;
            ...
            if (best.size() == 7) break;
        }
        if (best.size() == 7) append(best);
    }
    ...
    for (const auto index : order) sorted.push_back(std::move(solutions[index]));
